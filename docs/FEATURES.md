...
| 模擬成功／失敗付款 | 已實作 | 限 pending；既有測試未涵蓋 pay |
| 真正 ECPay 金流 | 未實作 | 僅 `.env.example` 預留變數 |
| 後台商品 CRUD | 已實作 | 6 個 API 測試；刪除可能受購物車外鍵阻擋 |
| 後台訂單篩選／詳情 | 已實作 | 4 個 API 測試；沒有修改訂單端點 |
| 運費 | 部分 | 前端顯示規則，後端總額未計入 |
...
購物車頁用 computed 自行算商品小計，未滿 NT$500 顯示運費 NT$150，滿額免運；結帳頁重複同規則。後端 cart.total／orders.total_amount 都只含商品小計，例如 300 元商品一件，前端總計顯示 450，建立訂單仍是 300。這是已知未完成整合，不能把前端顯示額作帳務結果。
